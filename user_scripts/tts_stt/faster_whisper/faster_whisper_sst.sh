...
if ! command -v pw-record &> /dev/null; then
    fatal "pw-record not found. Install 'pipewire' in your NixOS config."
fi
...
DEFAULT_SOURCE=$(wpctl status | grep -A 10 "Sources:" | grep "*" | awk '{print $3}')
...
pw-record --target="$DEFAULT_SOURCE" \
          --channels=1 \
          --rate=16000 \
          --latency=50ms \
          "$AUDIO_FILE" > "$REC_LOG" 2>&1 &
