...
}:
{
  ...
  system.activationScripts.postActivation.text = ''
    ...
    # Tor Browser: enter the tor network via the mesh relay on contabo
    # (microsocks, 10.100.0.1:1080). Written into the app-shipped
    # torrc-defaults — unlike TorBrowser-Data/Tor/torrc (regenerated by
    # the browser every launch), torrc-defaults is static input tor
    # honors. The activation script re-asserts after brew upgrades
    # replace the app bundle.
    f="/Applications/Tor Browser.app/Contents/Resources/TorBrowser/Tor/torrc-defaults"
    if [ -f "$f" ] && ! grep -q "^Socks5Proxy 10.100.0.1:1080" "$f"; then
      printf "\n# enter via mesh relay — asserted by darwin-configuration.nix\nSocks5Proxy 10.100.0.1:1080\n" >> "$f"
    fi
  '';
